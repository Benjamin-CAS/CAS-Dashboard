...
class HomeFragment : BaseFragment<FragmentHomeBinding,HomeFrgViewModel>(R.layout.fragment_home) {
    ...
    private val sideBarAdapter = SideBarAdapter { loginResultItem ->
        when(loginResultItem.type){
            "2" -> replaceFrg(MonitoringFragment(loginResultItem.id))
        }
    }
    ...
    private fun viewBindingApply() = with(binding){
        ...
        menuBtn.setOnClickListener { binding.drawerLayout.openDrawer(Gravity.START) }
    ...
    private fun replaceFrg(frg:Fragment) =
        childFragmentManager
            .beginTransaction()
            .replace(R.id.home_nav_host,frg)
            .commit()
    companion object {
